...
| R-09 | Container-to-host inference reachability was unverified, and was recorded as the most probable early impediment | **Closed by measurement, 2026-09-09.** The impediment does not occur on Docker Desktop: `host.docker.internal` resolves to its host proxy, which forwards to the host's loopback, so Ollama stays on its default binding and the anticipated `OLLAMA_HOST=0.0.0.0` mitigation is unnecessary. A Linux-native engine has no such proxy and still needs it, which is recorded in [NOVA-SPK-003](spikes/2026-09-09-container-to-host-reachability.md) finding 3 rather than left as a surprise |
...
| 0.4 | 2026-09-09 | Laxmi Poudel | R-09 closed by measurement on the evidence in NOVA-SPK-003 |
